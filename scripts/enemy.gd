...
func _ready():
	...
	#original_position_return.connect("timeout", Callable(self, "_on_original_position_return_timeout"))
...
func main_logic(delta):
	if is_being_hit:
		...
		#elif projectile_posi:
			#direction = projectile_posi - position
			#direction = direction.normalized()
			#last_direction = direction
			#angle = direction.angle()
			#run_back_to_origial_posi()
		...
		if player_chase:
			...
			if player_in_attack_cooldown==true:
				last_direction = direction
				var angle = direction.angle()
				# can change to idle or attack too
				play_directional_anim(angle, "idle")
...
#func _on_animated_sprite_2d_animation_finished() -> void:
	...
	#if current_anim in ["side_aggro", "front_aggro", "back_aggro"]:
...
func _on_timer_hit_timeout() -> void:
	...
	#get_tree().create_timer(1.0).timeout.connect(callback_func)
...
func _on_enemy_hitbox_body_entered(body: Node2D) -> void:
	...
	#if player_body_for_attack.has_method("take_player_damage"):
		#player_body_for_attack.take_player_damage(-attack_damage)
