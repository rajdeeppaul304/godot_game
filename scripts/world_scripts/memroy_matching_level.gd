...
func _ready() -> void:
	$Signpost.toggled_canvas.connect(_toggle_player_movement_and_hud_and_touch_control)

	$Signpost/CanvasLayer/Panel/MemoryMatching.game_won.connect(_game_won)
	$Signpost/CanvasLayer/Panel/MemoryMatching.game_lost.connect(_game_lost)
	
	
func _toggle_player_movement_and_hud_and_touch_control(canvas_bool):
	# doing opposite of canvas bool as it await for the animation to update
	$Player.is_movement_paused = !canvas_bool
	if canvas_bool:
		$Player.show_hud_touch_controls()
	elif not canvas_bool:
		$Player.hide_hud_touch_controls()
		
		
func next_scene():
	request_scene_change.emit("shooter_game_puzzle_3", "SpawnPoint1", true, "res://dialogues/test_dialogue_1.dialogue")  # Or whatever scene you want
...
func _game_lost():
	...
	await get_tree().create_timer(0.5).timeout
	$Player.death_player("moves_exhausted")
...
func open_gate():
	...
	var tween = create_tween()
	var material := $StaticBody2D2/Sprite2D.material as ShaderMaterial
	tween.tween_property(material, "shader_parameter/wipe_progress", 0, 1.0) # 1 second
