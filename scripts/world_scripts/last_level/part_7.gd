...
func next_scene():
	request_scene_change.emit("last_level_part_10", "spawnpoint1", false)

func previous_scene():
	request_scene_change.emit("last_level_part_9", "spawnpoint2", false) 

func previous_scene2():
	request_scene_change.emit("last_level_part_6", "spawnpoint2", false) 
...
func _ready() -> void:
	print(Manager.last_level_part_7_opened, "last_level_part_7_opened")
	if Manager.last_level_part_7_opened == true:
		$path_revealed.visible = true
		for child in $Area2D.get_children():
			if "to_remove" in child.name:
				child.queue_free()
